...
for start in range(len(start_locations)):
    ...
    print(wheel_moments_left_pin)    
    
    print("Reaction Forces\n---------------")
    # Sum of moments (0) = -(Train Moment) + Ry x 1200
    left_pin_reaction = wheel_moments_left_pin / L   # N 
    print("Left Pin: " + str(left_pin_reaction))
    
    # Sum of vertical forces.
    right_pin_reaction = P - left_pin_reaction
    print("Right Pin: " + str(right_pin_reaction) + "\n") 
...
center_line = np.array([0] * (n + 1))
left_sfd = np.array(left_sfd) 
middle_sfd = np.array(middle_sfd)
right_sfd = np.array(right_sfd)

shear_force_diagrams = [center_line, left_sfd, middle_sfd, right_sfd]
...
for sfd in range(len(shear_force_diagrams)): 
    ...
    if leg_label == None: 
        plt.plot(center_line, color="black")
    else:
        plt.plot(shear_force_diagrams[sfd], label = leg_label)
